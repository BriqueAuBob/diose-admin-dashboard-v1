...
onMounted(async () => {
    try {
        const { data } = await axios.get('/administration/permissions/' + route?.params?.id);
...
const update = async () => {
    try {
        ...
        await axios.put('/administration/permissions/' + route?.params?.id, form.permission);
        ElNotification({
            title: 'Succès',
...
const destroy = async () => {
    try {
        isLoading.value = true;
        await axios.delete('/administration/permissions/' + route?.params?.id, form.permission);
        ElNotification({
            title: 'Succès',
